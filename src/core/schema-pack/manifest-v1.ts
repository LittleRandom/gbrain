...
/**
 * v0.42 — ExtractableSpec widening. `extractable` is now `boolean | struct`.
 *
 ...
 *
 * Forward-compat: `verifier_path` is RESERVED in v0.42. The parser accepts
 * the field (validated as relative path within pack root by future logic)
 * but the runtime REFUSES to load pack-shipped verifier code in v0.42 —
 * v0.43 trust review is the gate. Pack authors who write the path early
 * get a clear runtime refuse message; they're not blocked at parse time.
 *
...
const ExtractableSpecSchema = z.object({
   ...
   * with NO conversation context per the v0.42 threat model. */
  ...
  /** RESERVED v0.43+: relative path to pack-shipped verifier code.
   * Validated as relative + within-pack at parse; REFUSES at runtime in
   * v0.42 with paste-ready hint. */
